...
class SceOAuthTransaction(models.Model):
    ...
    code_verifier_secret_id = fields.Many2one(
        "sce.credential.secret", required=True, ondelete="restrict"
    )
    ...
    status = fields.Selection(
        ...
        default="auth_pending",
        required=True,
    )

    _state_hash_unique = models.Constraint("UNIQUE(state_hash)", "El estado OAuth debe ser único.")
    ...
    def consume(self):
        self.ensure_one()
        self.env.cr.execute("SELECT id FROM sce_oauth_transaction WHERE id = %s FOR UPDATE", (self.id,))
        self.invalidate_recordset()
        if self.status != "auth_pending":
