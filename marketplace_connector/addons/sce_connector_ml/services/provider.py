# -*- coding: utf-8 -*-
"""MercadoLibre provider entry point for the SCE connector addon."""
...
class MercadoLibreExternalProvider(IProvider):
    """External MercadoLibre provider compatible with the SCE provider contract."""
    ...
    def capabilities(self):
        return {"oauth_exchange": True, "oauth_refresh": True, "health_check": True}
...
def get_provider(env, account):
    ...
    if provider_type != "mercadolibre":
        raise UserError(f"Invalid provider_type for sce_connector_ml: {provider_type}")
